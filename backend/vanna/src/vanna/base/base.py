...
class VannaBase(ABC):
    ...
    def generate_sql(self, question: str, allow_llm_to_see_data=False, **kwargs) -> str:
        ...
        self.log(title="SQL Prompt", message="生成了SQL查询")
        ...
        response = self.submit_prompt(
            [
                self.system_message(
                    ...
                    f"Users will give you SQL, and you will try to guess the business question the query answers using {self.config.get('language', 'English')}. Only return the question, do not include any additional explanations. Do not mention the table name in the question."
    ...
    def connect_to_bigquery(
        ...
        **Example:**
        ...
        if "google.colab" in sys.modules:
            ...
            logger.info("Not using Google Colab.")
        ...
        if not cred_file_path:
            try:
                ...
                logger.info("Could not found any google cloud implicit credentials")
        else:
    ...
    def connect_to_duckdb(self, url: str, init_sql: str = None, **kwargs):
        ...
        if url == ":memory:" or url == "":
            ...
            # Path to save the downloaded database
            logger.info(os.path.exists(url))
    ...
    def _get_databases(self) -> List[str]:
        try:
            logger.info("Trying INFORMATION_SCHEMA.DATABASES")
            df_databases = self.run_sql("SELECT * FROM INFORMATION_SCHEMA.DATABASES")
        except Exception as e:
            logger.info(e)
            try:
                logger.info("Trying SHOW DATABASES")
    ...
    def get_training_plan_snowflake(
        ...
        if use_historical_queries:
            try:
                logger.info("Trying query history")
        ...
        for database in databases:
            ...
            try:
                ...
                logger.info(f"Trying INFORMATION_SCHEMA.COLUMNS for {database}")

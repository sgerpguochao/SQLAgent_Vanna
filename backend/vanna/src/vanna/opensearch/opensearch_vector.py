...
class OpenSearch_VectorStore(VannaBase):
  def __init__(self, config=None):
    ...
    logger.info("OpenSearch_VectorStore initialized with document_index: ",
    ...
    logger.info("OpenSearch_VectorStore initialized with es_urls: ", es_urls,
    ...
    if es_urls is not None:
      ...
      logger.info("OpenSearch_VectorStore initialized with client over ")

    # 执行一个简单的查询来检查连接
    try:
      logger.info('Connected to OpenSearch cluster:')
      info = self.client.info()
      logger.info('OpenSearch cluster info:', info)
    except Exception as e:
      logger.info('Error connecting to OpenSearch cluster:', e)
  ...
  def remove_training_data(self, id: str, **kwargs) -> bool:
    try:
      ...
      logger.info("Error deleting training dataError deleting training data: ", e)
      return False
